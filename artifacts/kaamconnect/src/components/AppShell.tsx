...
export function Logo({ compact = false }: { compact?: boolean }) {
  return <Link href="/" data-testid="link-logo" className="flex items-center gap-2.5 text-[hsl(var(--sidebar-foreground))]"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[hsl(var(--sidebar-primary))] font-bold text-[hsl(var(--sidebar-primary-foreground))] shadow-[3px_3px_0_hsl(var(--sidebar-primary)/.22)]">K</span>{!compact && <span className="display-font text-xl font-bold tracking-tight">kaam<span className="text-[hsl(var(--sidebar-primary))]">connect</span></span>}</Link>;
}
  ...
  return <div className="noise min-h-[100dvh] bg-[hsl(var(--background))]">
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[248px] -translate-x-full flex-col bg-[hsl(var(--sidebar))] px-5 py-6 text-[hsl(var(--sidebar-foreground))] transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : ''}`}>
      ...
      <div className="mt-auto space-y-1 border-t border-[hsl(var(--sidebar-border))] pt-5">
        ...
        <div className="mt-4 flex items-center gap-3 rounded-xl bg-[hsl(var(--sidebar-accent))] p-3"><span className="grid h-8 w-8 place-items-center rounded-full bg-[#e4a3a0] text-xs font-bold text-[#19343a]">NS</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">Nisha Shah</p><p className="text-[10px] text-[hsl(var(--sidebar-foreground)/.56)]">Both modes active</p></div><button onClick={() => setToast('Signed out of this demo')} data-testid="button-sign-out" className="text-[hsl(var(--sidebar-foreground)/.55)]"><LogOut size={14} /></button></div>
      </div>
    </aside>
    ...
    <main className="min-h-[100dvh] lg:pl-[248px]">
      <header className="sticky top-0 z-20 flex h-[72px] items-center justify-between border-b border-[hsl(var(--border)/.8)] bg-[hsl(var(--background)/.92)] px-4 backdrop-blur-md sm:px-8">
        ...
        <div className="flex items-center gap-2 sm:gap-4"><button onClick={() => setToast('You are all caught up')} className="relative rounded-xl p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]" data-testid="button-notifications"><Bell size={19} /><span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-[hsl(var(--accent))]" /></button><Link href="/settings" className="hidden items-center gap-2 rounded-xl border border-[hsl(var(--border))] px-2 py-1.5 sm:flex" data-testid="link-header-profile"><span className="grid h-7 w-7 place-items-center rounded-full bg-[#e4a3a0] text-[10px] font-bold">NS</span><span className="text-xs font-semibold">Nisha</span><ChevronDown size={13} /></Link></div>
      </header>
      <div className="mx-auto max-w-[1380px] px-4 py-6 sm:px-8 sm:py-8">{children}</div>
    </main>
    {notice && <div className="fixed bottom-5 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-full bg-[hsl(var(--foreground))] px-4 py-3 text-sm font-semibold text-[hsl(var(--background))] shadow-xl rise-in" data-testid="status-toast"><Sparkles size={15} className="text-[hsl(var(--secondary))]" />{notice}</div>}
  </div>;
}
...
export function Avatar({ initials, color = 'bg-[#f1c76f]', size = 'md' }: { initials: string; color?: string; size?: 'sm' | 'md' | 'lg' }) {
  return <span className={`grid shrink-0 place-items-center rounded-full font-bold text-[#19343a] ${color} ${size === 'sm' ? 'h-8 w-8 text-[10px]' : size === 'lg' ? 'h-16 w-16 text-lg' : 'h-11 w-11 text-xs'}`}>{initials}</span>;
}

export function Badge({ children, tone = 'neutral' }: { children: ReactNode; tone?: 'neutral' | 'green' | 'yellow' | 'coral' }) {
  const tones = { neutral: 'bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground))]', green: 'bg-[#d7eee4] text-[#17644e]', yellow: 'bg-[#fff0bf] text-[#805f08]', coral: 'bg-[#f8ddd7] text-[#9c382d]' };
...
export function Button({ children, variant = 'primary', className = '', onClick, href, type = 'button', disabled = false, testId }: { children: ReactNode; variant?: 'primary' | 'secondary' | 'ghost' | 'danger'; className?: string; onClick?: () => void; href?: string; type?: 'button' | 'submit'; disabled?: boolean; testId?: string }) {
  const styles = { primary: 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] shadow-[3px_3px_0_hsl(var(--primary)/.2)] hover:-translate-y-0.5', secondary: 'bg-[hsl(var(--secondary))] text-[hsl(var(--secondary-foreground))] shadow-[3px_3px_0_hsl(var(--secondary)/.35)] hover:-translate-y-0.5', ghost: 'border border-[hsl(var(--border))] bg-[hsl(var(--card))] hover:bg-[hsl(var(--muted))]', danger: 'bg-[hsl(var(--destructive))] text-white' };
  ...
  return <button type={type} disabled={disabled} onClick={onClick} data-testid={testId} className={`inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-50 ${styles[variant]} ${className}`}>{children}</button>;
}

export function StatCard({ label, value, detail, accent = 'teal' }: { label: string; value: string; detail: string; accent?: 'teal' | 'yellow' | 'coral' }) {
  const color = { teal: 'bg-[#d7eee4]', yellow: 'bg-[#fff0bf]', coral: 'bg-[#f8ddd7]' }[accent];
  return <div className={`rounded-2xl ${color} p-5`}><p className="text-xs font-semibold text-[hsl(var(--foreground)/.65)]">{label}</p><p className="display-font mt-3 text-3xl font-bold">{value}</p><p className="mt-1 text-xs text-[hsl(var(--foreground)/.62)]">{detail}</p></div>;
}